...
var installPlugin = function (name, cb) {
  ...
  if (fs.existsSync('plugins/' + name)) {
    winston.info("[Plugin] Can't install plugin already installed");
    cb();
    return;
  }
  ...
  request({
    ...
    if (isFolder) {
      ...
      try {
        fs.renameSync(tmp, 'plugins/' + name);
      } catch (ex) {}
    }
    cb();
...
var getFiles = function (name) {
  ...
  if (fs.existsSync(path + '.xml')) {
    list.push(name + '.xml');
  }
  if (fs.existsSync(path + '.prop')) {
    list.push(name + '.prop');
  }
