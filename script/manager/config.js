...
var loadProperties = function () {
  if (!fs.existsSync('script/wsrnode.prop')) {
    return {};
  }
...
var retains = function (source, target) {
  ...
  Object.keys(source).forEach(function (attr) {
    if (attr == 'description' || attr == 'version') {
      return false;
    }
    if (target[attr] === undefined &&
      ...
      attr != 'w' && attr != 'h' &&
      attr != 'c') {
      return winston.log('warn', 'Bypass config: ', attr);
    }
...
var getTicker = function () {
  ...
  request({
    ...
    if (err || response.statusCode != 200) {
      winston.info("Can't retrieve remote ticker");
      return;
    }
...
var ConfigManager = {
  ...
  'load': function () {
    ...
    try {
      ...
      winston.log('error', 'Error while loding properties:', ex.message);
    }
    return ConfigManager;
  },
